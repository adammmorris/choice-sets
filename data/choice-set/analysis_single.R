...
for (i in 1:nrow(df.s1)) {
  ...
  df.s1$correct_choice[i] = choice == v1
...
for (subj in 1:length(subjlist)) {
  ...
  exclude = df.demo.temp$write_down == 'Yes' || df.s2.subj.temp$comp_check_pass < 1 ||
    ...
    df.s1.subj.temp$numTrials != numTrials || df.s1.subj.temp$pctCorrect_choice < .7
  if (exclude) {
...
ggplot(df.graph.s1, aes(x = high_s1value, y = in.cs)) +
  ...
  scale_y_continuous(breaks = c(.35,.45), limits = c(.35,.45)) + 
...
m.s1.cs = lmer(cs.order~s1_value+(1+s1_value||subject),
                data = df.words.filt %>% filter(s1_value >= 4))
summary(m.s1.cs)
...
df.demo = df.demo %>% filter(subject != 'A18DT5HTL9O18K') %>% mutate(s2_bonus = I(df.s2.subj$s2_bonus), mem_bonus = I(df.s2.subj$mem_bonus),
