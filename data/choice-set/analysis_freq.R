  ...
  if (length(cs) > 0) {
    ...
    for (j in 1:length(cs)) {
      ...
      if (length(choice) > 0) {
        ...
        df.words$cs.order[word_rows] = NA
      }
    }
  } else {
    df.words$in.cs[word_rows] = NA
...
for (subj in 1:length(subjlist)) {
  ...
  exclude = df.demo.temp$write_down == 'Yes' || df.s2.subj.temp$comp_check_pass < 1 ||
    ...
    df.s1.temp$pctCorrect_choice < .7 #|| (df.demo.temp$tr_rt + df.demo.temp$tr_resp + df.demo.temp$tr_resp2) < 3
  if (exclude) {
...
ggplot(df.words.filt %>% filter(cond > 0), aes(x = num_chosen, y = avg_rel)) +
  geom_point()

test = df.words.filt %>% filter(cond == 0) %>% mutate(often = factor(num_chosen > 10, c(F,T), c('Rare', 'Often')))
test2 = test %>% group_by(often, subject) %>% summarize(in.cs = mean(in.cs)) %>% group_by(often) %>%
  summarize(in.cs = mean(in.cs, na.rm = T), in.cs.se = sqrt(in.cs * (1-in.cs) / n()))
ggplot(test2, aes(x = often, y = in.cs)) + geom_point(size = 5) +
...
df.graph.s1 = df.words.filt %>% mutate(often = factor(num_chosen > 10, c(F,T), c('Rare', 'Often'))) %>%
  group_by(often) %>% filter(cond > 0) %>%
  summarize(in.cs = mean(in.cs, na.rm = T), in.cs.se = sqrt(in.cs * (1-in.cs) / n()),
            ...
            num.chosen.m = mean(num_chosen), num.chosen.se = se(num_chosen))
# choice sets
ggplot(df.graph.s1, aes(x = often, y = in.cs)) +
  ...
  #facet_wrap(~cond)
# order
ggplot(df.graph.s1, aes(x = s1_value, y = cs.order.m)) +
  geom_point(size = 5) + geom_line() +
  geom_errorbar(aes(ymin = cs.order.m - cs.order.se, ymax = cs.order.m+cs.order.se), width = .2) #+
  #geom_smooth(method='lm')
# selection out of choice set
ggplot(df.graph.s1, aes(x = cond, y = chosen)) +
  geom_point(size = 5) + geom_line() +
  ...
  #scale_x_continuous(breaks = c(1,12))# +
  #facet_wrap(~cond)
# num times chosen in S1
ggplot(df.graph.s1, aes(x = cond, y = num.chosen.m)) +
  geom_point(size = 5) + geom_line() +
  geom_errorbar(aes(ymin = num.chosen.m - num.chosen.se, ymax = num.chosen.m+num.chosen.se), width = .2)

# 2x2
df.graph.s1 = df.words.filt %>% filter(cond == 0) %>% #mutate(#high_s1value = factor(high_s1value, c(F,T), c('Low', 'High')),
...
df.graph.s2 = df.words.filt %>% group_by(s2_value) %>%
  summarize(in.cs = mean(in.cs, na.rm = T), in.cs.se = sqrt(in.cs * (1-in.cs) / n()),
            chosen = mean(chosen, na.rm = T), chosen.se = sqrt(chosen * (1-chosen) / n()))
# choice sets
...
m.s1.cs = lmer(in.cs~num_chosen+(num_chosen|subject),
                data = df.words.filt %>% filter(cond > 0))
summary(m.s1.cs)
...
df.modeling = df.s2 %>% filter(
  subject %in% include_names & !is.na(choice_real_ind)) %>%
...
df.s2.subj = df.s2 %>% filter(question_order == 0 & stamp > '2019-01-15' & stamp < '2019-01-16' & subject != 'A3JKTMLUUCR46A') %>%
  mutate(s2_bonus = ifelse(is.na(s2_value), 0, s2_value),
         mem_bonus = 0)
df.demo = df.demo %>% filter(stamp > '2019-01-15' & stamp < '2019-01-16' & subject != 'A3JKTMLUUCR46A') %>% mutate(s2_bonus = I(df.s2.subj$s2_bonus), mem_bonus = I(df.s2.subj$mem_bonus),
