...
for (i in 1:nrow(df.s1)) {
  ...
  df.s1$correct_word[i] = ain(toupper(df.s1$resp[i]), df.s1$word_chosen[i], maxDist = 2)
  df.s1$correct_val[i] = df.s1$resp2[i] == df.s1$value[i]
