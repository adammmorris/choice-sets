...
for (subj in 1:length(subjlist)) {
  ...
  exclude = df.demo.temp$write_down == 'Yes' || df.s2.subj.temp$comp_check_pass < 1 ||
    ...
    df.s1.subj.temp$pctCorrect_choice < .7 || sum(recalled[subj,]) < 5
  if (exclude) {
...
df.graph = df.s2.filt %>%
  mutate(choice_type = factor(choice_real_ind, c(1,2,3,4,5:12), c('high_s1', 'mid_s1', 'low_s1', 'high_s2', rep('distractor', 8)))) %>%
