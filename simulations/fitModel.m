...
bounds = [2 0 0 0 0 0 0; 4 10 10 10 10 1 1];
WEIGHT_INDS = [false false false false false true false];
...
for whichSubj = 1:numSubjects
    ...
    recalled_subj = recalled(whichSubj, :);
    ...
    if nFreeParams > 0 % Are there free parameters?
        if fixedParams(K_PARAM_IND) == -1 % Are we optimizing over the discrete nToEval parameter?
            ...
            for nToEval_ind = 1:nDiscrete
                if nContFreeParams > 0 % If there's other continuous parameters to optimize over..
                    f = @(params) -getPosterior(choice(index), rewards_s1_subj, rewards_s2_subj, recalled_subj, [krange(nToEval_ind) params], fixedParams, priorPDFs, nSamples);
                    ...
                    logposts(nToEval_ind) = getPosterior(choice(index), rewards_s1_subj, rewards_s2_subj, recalled_subj, krange(nToEval_ind), fixedParams, priorPDFs, nSamples);
            ...
            f = @(params) -getPosterior(choice(index), rewards_s1_subj, rewards_s2_subj, recalled_subj, params, fixedParams, priorPDFs, nSamples);
            logposts_starts = zeros(numStarts, 1);
        ...
        ll = getLikelihood(choice(index), rewards_s1_subj, rewards_s2_subj, recalled_subj, optParams, fixedParams, nSamples);
        ...
        [post, ll] = getPosterior(choice(index), rewards_s1_subj, rewards_s2_subj, recalled_subj, [], fixedParams, priorPDFs, nSamples);
        bic = -2 * ll;
        lme = ll;
