function [results, params] = fit(dataPath, fixedParams, priorPDFs, numStarts, nFnEvals)
...
for whichSubj = 1:numSubjects
    ...
    if nFreeParams > 0 % Are there free parameters?
        if fixedParams(K_PARAM_IND) == -1 % Are we optimizing over the discrete nToEval parameter?
            ...
            for nToEval_ind = 1:nDiscrete
                if nContFreeParams > 0 % If there's other continuous parameters to optimize over..
                    f = @(params) -posterior(choice(index), rewards_s1_subj, rewards_s2_subj, recalled_subj, [krange(nToEval_ind) params], fixedParams, priorPDFs);
            ...
            f = @(params) -posterior(choice(index), rewards_s1_subj, rewards_s2_subj, recalled_subj, params, fixedParams, priorPDFs);
            logposts_starts = zeros(numStarts, 1);
        ...
        ll = likelihood(choice(index), rewards_s1_subj, rewards_s2_subj, recalled_subj, optParams, fixedParams);
        ...
        [post, ll] = posterior(choice(index), rewards_s1_subj, rewards_s2_subj, recalled_subj, [], fixedParams, priorPDFs);
        bic = -2 * ll;
        lme = ll;
