...
if nToEval == 1 % any non-choice-set
    for trial = 1:numTrials
        ...
        probs_num = exp(beta * ...
                ...
                exposures_tr(availWords) * w_MF / maxExposures_tr + ...
                rewards_tr(availWords) * w_NE / maxRe_tr + ...
    ...
    for trial = 1:numTrials
        ...
        for numTies = 0:(nToEval - 1)
            prob = prob + binCoef(lq, nToEval - numTies - 1) * binCoef(tq, numTies) / ...
                ((numTies + 1) * binCoef(numAvailWords, nToEval));
        end
        ...
        for trial = 1:numTrials
            ...
            for numTies = 0:(nToEval-1)
                ...
                for set_ind = 1:size(sets, 1)
                    ...
                    prob = prob + (1 / (numTies + 1)) * (sum(temp_prob) + 1 + d * ((-1)^numel(set)));
                end
            end
